...
namespace AccCreationAPI.Controllers
{
    ...
    public class LoginController : ControllerBase
    {
        ...
        private string GenerateAccessToken(string userName, Dictionary<string, string> roles)
        {
            var securityKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(_configuration.GetSection("Jwt:Key").Value!));
            var credentials = new SigningCredentials(securityKey, SecurityAlgorithms.HmacSha256);
        ...
        new Claim(JwtRegisteredClaimNames.Jti, Guid.NewGuid().ToString()) // Unique identifier for the token
            ...
            // Add role claims to the token based on the dictionary
            foreach (var roleEntry in roles)
            {
                ...
                // Handle other key-value pairs in the dictionary if necessary
            }
